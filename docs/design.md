...
**Two audiences, one site (UX recommendation, accepted 2026-09-24).** Visitors are never asked to choose a path. Hiring managers usually arrive through a shared link to a tool, case study, or About page, so a "Try it in 2 minutes" link sits in every page's header and goes straight to the rationalization exercise. The home page opens with that link and a case-study teaser, then a short teaser for the lifecycle wheel, then tool depth. The wheel itself lives on its own page (`site/lifecycle.html`), not buried in the home page's scroll (decided 2026-09-24, after Eve reviewed the first build). Navigation order: Tools, Lifecycle, Crosswalk explorer, Case studies, About — Tools stays first for repeat visitors. There is no login: the working path stays private by where it lives (Copilot in the employer's Microsoft 365, local Claude and Codex, a private repo for notes).
...
**Crosswalk-to-stage impact (added 2026-09-24).** Each crosswalk theme also names which lifecycle stages it primarily governs and which it only touches in passing — a judgment call, not derived mechanically from the question bank (a theme can be primary at a stage that has no question citing it yet, and secondary at the one stage that does). Recorded in `methods/crosswalk/lifecycle-impact.csv` (`xw_id, primary_stages, secondary_stages`, stage slugs semicolon-separated), checked at build time against real XW ids and stage slugs. The crosswalk explorer's detail panel shows a "Where this applies" line: primary stages as bold links to that stage on the lifecycle page, secondary stages as muted links. The lifecycle page accepts a stage in its URL hash (`lifecycle.html#deploy`) to open pre-selected, the same pattern the crosswalk explorer now uses for a row.
